...
def test_inbreeding_inbred_common_ancestor():
    ...
    sire_names =  [None, None, None, "A", "A", None, "C", "E", "G", None, None, "I", "I", None, "K", "M", "O", "O"]
    dam_names = [None, None, None, "B", "B", None, "D", "F", "H", None, None, "J", "J", None, "L", "N", "P", "P"]
    ...
    assert len({sire for sire in sire_names if sire is not None} & {dam for dam in dam_names if dam is not None}) == 0, "sires and dams are not disjoint"
    F = pedigree.inbreeding_coeffs(sires, dams)
...
def test_inbreeding_without_inbred_common_ancestor():
    ...
    sire_names =  [None, None, None, "A", "A", None, "C", "E", None, "G", "G", None, None, "K", "M"]
    dam_names = [None, None, None, "B", "B", None, "D", "F", None, "H", "H", None, "J", "L", "N"]
    sires = np.asarray([names.find(x) + 1 if x is not None else 0 for x in sire_names])
    dams = np.asarray([names.find(x) + 1 if x is not None else 0 for x in dam_names])
    assert len({sire for sire in sire_names if sire is not None} & {dam for dam in dam_names if dam is not None}) == 0, "sires and dams are not disjoint"
    F = pedigree.inbreeding_coeffs(sires, dams)
...
def test_inbreeding_18th_dynasty():
    ...
    names = ["Sequenenra III", "Aahotep I", "Aahmes", "Aahmes-Nefertari", "Senseneb", "Amenhotep I", "Aahotep II", "Thotmes I", "Aames", "Hatsheput"]
    sire_names =  [None, None, "Sequenenra III", "Sequenenra III", None, "Aahmes", "Aahmes", "Amenhotep I", "Amenhotep I", "Thotmes I"]
    dam_names = [None, None, "Aahotep I", "Aahotep I", None, "Aahmes-Nefertari", "Aahmes-Nefertari", "Senseneb", "Aahotep II", "Aames"]
...
def test_inbreeding_direct_collateral():
    ...
    names = "ABCDEFGHIJKLMNO"
    sire_names =  [None, None, None, None, None, "A", "C", "C", "E", "G", None, "J", "J", "L", "L"]
    dam_names = [None, None, None, None, None, "B", "B", "D", "F", "H", None, "I", "K", "M", "N"]
    sires = np.asarray([names.find(x) + 1 if x is not None else 0 for x in sire_names])
    dams = np.asarray([names.find(x) + 1 if x is not None else 0 for x in dam_names])
    assert len({sire for sire in sire_names if sire is not None} & {dam for dam in dam_names if dam is not None}) == 0, "sires and dams are not disjoint"
    F = pedigree.inbreeding_coeffs(sires, dams)
